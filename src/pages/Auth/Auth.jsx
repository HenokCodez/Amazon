...
function Auth() {
  ...
  const authHandler = (e) => {
    ...
    if (e.target.name === "signIn") {
      ...
      setLoading({ ...loading, signIn: true }); // Set loading state for sign in (it will load/rotate)
      ...
      signInWithEmailAndPassword(auth, email, password)
        .then((userInfo) => {
          ...
          setLoading({ ...loading, signIn: false }); // Reset loading state (it will stop loading/rotating)
  ...
  return (
    <section className={classes.auth}>
      {/* Amazon logo */}
      <Link to="/">
        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/0/06/Amazon_2024.svg/1280px-Amazon_2024.svg.png" alt="amazon logo" />
      </Link>
      ...
      <div className={classes.auth__container}>
        ...
        {navStateData?.state?.msg && <small className={classes.auth__msg_login}>{navStateData?.state?.msg}</small>}
        ...
        <form>
          ...
          {/* Sign In button */}
          <button type="submit" onClick={authHandler} name="signIn" className={classes.auth__signIn_btn}>
            {loading.signIn ? <ClipLoader color="#000" size={15} /> : "Sign In"} {/* Display loading spinner or text */}
          </button>
        </form>

        {/* Agreement message */}
        <p>By signing-in you agree to the AMAZON FAKE CLONE Conditions of Use & Sale. Please see our Privacy Notice, Cookies Notice, and our Interest-based Ads Notice.</p>

        {/* Create Account button */}
        <button type="submit" onClick={authHandler} name="signUp" className={classes.auth__signUp_btn}>
          {loading.signUp ? <ClipLoader color="#000" size={15} /> : "Create your Amazon Account"} {/* Display loading spinner or text */}
        </button>
